...
namespace HospitalX
{
    public partial class ucRole : UserControl
    {
        ...
        // thì thêm RoleItem mới vào _roles và refresh danh sách
        // ================================================
        private void BtnCreate_Click(object sender, EventArgs e)
        {
            using (var frm = new frmCreateRole())
            {
                ...
                {
                    _roles.Add(new RoleItem(frm.RoleName, "Role mới tạo", 0, 0, Color.FromArgb(240, 248, 255), Color.FromArgb(0, 120, 180)));

                    // Refresh list
                    txtSearch.Text = ""; // Reset filter -> triggers TextChanged -> reload
                }
            }
        }

        // ================================================
        // MOCK DATA — tạo 4 role mẫu với thành viên và quyền
        // tương ứng với 4 vai trò trong hệ thống bệnh viện.
        // Sau này thay bằng truy vấn Oracle: DBA_ROLES, DBA_ROLE_PRIVS
        // ================================================
        private void LoadMockData()
        {
            var r1 = new RoleItem("ROLE_BACSI", "Bác sĩ / Y sĩ", 2, 4, Color.FromArgb(230, 240, 255), Color.FromArgb(0, 50, 150));
            r1.Members.AddRange(new[] {
                new MemberItem("U_BACSI01", "BA", Color.FromArgb(41, 121, 255)),
                new MemberItem("U_BACSI02", "B2", Color.FromArgb(38, 166, 154))
            });
            r1.Privileges.AddRange(new[] {
                new PrivItem("NHANVIEN", "TABLE", new string[] { "SELECT" }),
                new PrivItem("HSBA", "TABLE", new string[] { "SELECT", "UPDATE" }),
                new PrivItem("DONTHUOC", "TABLE", new string[] { "SELECT", "INSERT" }),
                new PrivItem("SP_CAP_NHAT_HSBA", "PROCEDURE", new string[] { "EXECUTE" })
            });

            var r2 = new RoleItem("ROLE_DIEUPHOIVIEN", "Điều phối viên", 1, 3, Color.FromArgb(250, 240, 245), Color.OrangeRed);
            r2.Members.Add(new MemberItem("U_DIEUPHOI01", "DP", Color.OrangeRed));
            r2.Privileges.AddRange(new[] {
                new PrivItem("PHONGBAN", "TABLE", new string[] { "SELECT" }),
                new PrivItem("LICH_TRUC", "TABLE", new string[] { "SELECT", "UPDATE", "INSERT" }),
                new PrivItem("NHANVIEN", "TABLE", new string[] { "SELECT" })
            });

            var r3 = new RoleItem("ROLE_KYTHUATVIEN", "Kỹ thuật viên", 1, 2, Color.FromArgb(255, 250, 230), Color.DarkGoldenrod);
            r3.Members.Add(new MemberItem("U_KYTHUAT01", "KT", Color.DarkGoldenrod));
            r3.Privileges.AddRange(new[] {
                new PrivItem("XETNGHIEM", "TABLE", new string[] { "SELECT", "INSERT" }),
                new PrivItem("THIETBIT", "TABLE", new string[] { "SELECT" })
            });

            var r4 = new RoleItem("ROLE_BENHNHAN", "Bệnh nhân", 1, 1, Color.FromArgb(240, 255, 245), Color.SeaGreen);
            r4.Members.Add(new MemberItem("U_BN099", "BN", Color.SeaGreen));
            r4.Privileges.Add(new PrivItem("HSBA_VIEW", "VIEW", new string[] { "SELECT" }));

            // Add all roles
            _roles = new List<RoleItem> { r1, r2, r3, r4 };
            foreach (var r in _roles) lstRoles.Items.Add(r);

            UpdateFooter();

            // Trigger selection logically
            if (lstRoles.Items.Count > 0)
                lstRoles.SelectedIndex = 0;
        }
        ...
        private void LstRoles_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (lstRoles.SelectedItem is RoleItem item)
            {
                ...
                lblStatMemberVal.Text = item.MemberCount.ToString();
                lblStatPrivVal.Text = item.PrivilegeCount.ToString();
        ...
        private void LstRoles_DrawItem(object sender, DrawItemEventArgs e)
        {
            ...
            int badgeW1 = (int)memberSize.Width + 16;
            int badgeH = 22;
            int badgeX1 = e.Bounds.X + 350;
            ...
            int badgeW2 = (int)privSize.Width + 16;
            int badgeX2 = e.Bounds.X + 500;
    ...
    public class RoleItem
    {
        ...
        public RoleItem(string n, string sub, int m, int p, Color bg, Color ic)
        {
            Name = n; Subtitle = sub; MemberCount = m; PrivilegeCount = p; BgColor = bg; IconColor = ic;
